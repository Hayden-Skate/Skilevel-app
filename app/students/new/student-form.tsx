  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { data: account, error: accountError } = await supabase.from('users').upsert(
        { clerk_user_id: user.id, email, role: 'student_parent' }, { onConflict: 'clerk_user_id' },
      ).select('id').single();
      ...
      const message = caught instanceof Error ? caught.message : typeof caught === 'object' && caught && 'message' in caught ? String(caught.message) : 'We could not add this student.';
